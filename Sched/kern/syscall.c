...
sys_exofork(void)
{
	...
	newenv->tickets = curenv->tickets;   // todo lo mismo que el padre
	newenv->stride = curenv->stride;
	newenv->pass = curenv->pass;
...
//Reduce current process' number of ticket. Returns -1 if there is an attempt to increase priority.
static int
sys_reduce_priority(uint32_t new_priority)
{ 
	if (new_priority < curenv->pass)
		return -1;
	curenv->pass = new_priority;
	return 0;
}

//Returns current's process pass.
static int
sys_priority()
{
	return curenv->pass;
