...
void imprimir_mensajes_pipe(int fds_escritura_padre[CANALES],
                            int fds_escritura_hijo[CANALES]);
void imprimir_mensajes_en_comun(int fk);
void realizar_procesos_padre(int fds_escritura_padre[CANALES],
                             int fds_escritura_hijo[CANALES]);
void realizar_procesos_hijo(int fds_escritura_padre[CANALES],
                            int fds_escritura_hijo[CANALES]);
...
void
realizar_procesos_padre(int fds_escritura_padre[CANALES],
                        int fds_escritura_hijo[CANALES])
{
	int mensaje = rand();
	int recibido = 0;
...
void
realizar_procesos_hijo(int fds_escritura_padre[CANALES],
                       int fds_escritura_hijo[CANALES])
{
	int recibido = 0;
...
void
cerrar_fds(int fds_escritura_padre[CANALES], int fds_escritura_hijo[CANALES])
{
...
main()
{
	...
	if (pipe_escritura_padre == ERROR) {
		perror("Error al crear pipe de escritura para el proceso "
		       "padre.");
		return ERROR;
	}
