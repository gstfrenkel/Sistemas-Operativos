...
static char *
expand_magic_var(char *arg)
{
	char *aux = realloc(arg, sizeof(int));

	if (!aux) {
		perror("Error al expandiar variable de entorno");
		arg[0] = FIN_DE_STRING;
	} else {
		arg = aux;
...
expand_environ_var(char *arg)
{
	size_t lgth = 0;
	char *aux = NULL;
	char *aux_realloc = NULL;

	if (block_contains(arg, '$') == -1)
		return arg;

	else if (arg[0] == '$' && arg[1] == '?')
		return expand_magic_var(arg);

	else if (!(aux = getenv(arg + 1))) {
		arg[0] = FIN_DE_STRING;
		return arg;
	}

	if ((lgth = strlen(aux)) > strlen(arg)) {
		aux_realloc = realloc(arg, (lgth + 1) * sizeof(char));

		if (!aux_realloc) {
			perror("Error al expandir variable de entorno");
			arg[0] = FIN_DE_STRING;
			return arg;
		}
